...
export default function PeopleCreateForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(API.CREATE_EMPLOYEE, payload, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  return (
    <Box sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
      <Box sx={{ flex: 1, overflowY: "auto", p: { xs: 2, md: 3 }, bgcolor: "#f8fafc" }}>
        ...
        >
          <Box sx={{ px: 3, py: 2.25 }}>
            <Typography variant="h6" fontWeight={800}>Create Employee</Typography>
            <Typography variant="body2" color="text.secondary">Add a new employee to the system</Typography>
          </Box>
          ...
          <Box component="form" onSubmit={handleSubmit} sx={{ p: 3 }}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={6}>
                <TextField label="Employee Code" name="employee_code" value={form.employee_code} onChange={handleChange} required size="small" fullWidth />
              </Grid>
              <Grid item xs={12} md={6}>
            ...
            <Stack direction="row" spacing={1.5} justifyContent="flex-end" sx={{ mt: 3 }}>
              ...
              <Button type="submit" variant="contained" disabled={loading}>
                {loading ? <CircularProgress size={18} color="inherit" /> : "Create Employee"}
              </Button>
            </Stack>
          </Box>
        </Paper>
