// ReportsSidebar.jsx — MUI version
...
export default function ReportsSidebar({ visible, isOpen, setShow, anchorTop }) {
  const { user } = useAuth();

  const filteredMenu = reportsMenu.filter((item) => {
    if (item.path === "/reports/activity" && user?.role?.toLowerCase() !== "superadmin") {
      return false;
    }
    return true;
  });
    ...
    return (
      <Box sx={{ display: "flex", flexDirection: "column", mt: 0.5, mb: 0.5 }}>
        {filteredMenu.map((item, i) => {
          ...
          return (
            <NavLink key={i} to={item.path} end={item.path === "/reports"} style={{ textDecoration: "none" }}>
              {({ isActive }) => (
                <Box
                  sx={{
                    display: "flex",
  ...
  const PADDING       = 16;
  const contentHeight = HEADER_HEIGHT + filteredMenu.length * ITEM_HEIGHT + PADDING;
