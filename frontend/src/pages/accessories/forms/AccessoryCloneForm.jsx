...
export default function AccessoriesCloneForm() {
  const navigate = useNavigate();
  const { state } = useLocation();
  const accessory = state?.data || {};
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(API.CREATE_ACCESSORY, formData, {
        headers: {
      ...
      toast.success("Accessory cloned successfully!");
      navigate("/accessories");
    } catch (err) {
      console.error("Clone accessory failed:", err.response?.data || err);
      toast.error(
        JSON.stringify(
          err?.response?.data?.detail || err?.response?.data || "Failed to clone accessory",
          null,
          2
        )
      );
      ...
      <div className="max-w-5xl mx-auto bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm">
        <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800">
          ...
          <p className="text-xs text-zinc-400 mt-1">
            {accessory.name || "Accessory"} • Create duplicate with new details
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="flex gap-3 pt-4 border-t border-zinc-200 dark:border-zinc-800">
            ...
            >
              {loading ? "Cloning..." : "Clone Accessory"}
            </button>
