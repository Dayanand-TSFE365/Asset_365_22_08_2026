// pages/jobs/common/DataTable.jsx
...
function CustomToolbar({ ctxRef }) {
  ...
  return (
    ...
    >
      {/* LEFT: optional extra (e.g. FY filter) + Search */}
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
        ...
        {(() => {
          const canExport = hasPermission("export_jobs");
          return (
            <Select
              size="small"
              value="Export"
...
export default function DataTable({
  ...
  toolbarLeft,          // ← optional JSX rendered left of the search box
  isRowSelectable,      // ← optional (params) => boolean, lets callers block
                         //   checkbox selection on synthetic rows (e.g. panel
                         //   sub-rows flattened in by the caller)
  getRowClassName,      // ← optional (params) => string, lets callers style
                         //   synthetic rows differently (e.g. indent/shade
                         //   panel sub-rows)
  getRowHeight,         // ← optional (params) => number|null, per-row height
}) {
  ...
  exportStyledExcel(headers, cleanData, "Jobs", "jobs.xlsx");
  ...
  ctxRef.current = {
    ...
    containerRef,
    toolbarLeft,   // ← passed through to toolbar
  };
    ...
    >
      <DataGrid
        ...
        sx={{
          ...
          "& .panel-subrow":                   { bgcolor: "#f8fafc" },
          "& .panel-subrow:hover":             { bgcolor: "#f1f5f9" },
        }}
      />
    </Paper>
  );
