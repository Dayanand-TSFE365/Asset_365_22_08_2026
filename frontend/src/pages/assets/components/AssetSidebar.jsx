// AssetSidebar.jsx — MUI version
import { NavLink } from "react-router-dom";
import { assetsMenu } from "../assetsMenu";
import { Box, Typography, Paper } from "@mui/material";

// ── Design tokens (match Sidebar.jsx) ────────────────────────────────────────
...
const COLLAPSED_SIDEBAR_WIDTH = 72; // px — must match Sidebar.jsx SIDEBAR_WIDTH_CLOSED
const FLYOUT_WIDTH            = 220; // px

// Shared NavLink item styles
function SubItem({ item, pl = 5 }) {
...
export default function AssetSidebar({ visible, isOpen, setShow, anchorTop }) {

  // ── EXPANDED MODE: inline accordion inside main sidebar ──────────────────
  if (isOpen) {
    if (!visible) return null;
    return (
      <Box sx={{ display: "flex", flexDirection: "column", mt: 0.5, mb: 0.5 }}>
        {assetsMenu.map((item, i) => (
          <SubItem key={i} item={item} pl={5} />
        ))}
      </Box>
    );
  }

  // ── COLLAPSED MODE: flyout panel anchored next to the hovered item ────────
  // Estimate flyout content height so we can clamp it above the bottom of the screen
  const ITEM_HEIGHT    = 38;  // px per menu row
  const HEADER_HEIGHT  = 44;  // px for the "Assets" title bar
  const PADDING        = 16;  // top+bottom inner padding
  const contentHeight  = HEADER_HEIGHT + assetsMenu.length * ITEM_HEIGHT + PADDING;

  // Clamp so the panel never goes below the viewport
