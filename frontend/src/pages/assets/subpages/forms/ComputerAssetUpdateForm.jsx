...
// ─── ModalWrapper — outside parent to prevent remount blink ──────────────────
...
// ─── SelectWithNew — outside parent to prevent remount blink ─────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
export default function ComputerAssetUpdateForm() {
  ...
  // ── prefill form from row data ──────────────────────────────────────────
  // Note: in the row mapping, manufacturer => manufacturer_id and supplier => supplier_id
  // purchase_cost is stored as "₹1234" string so we strip the ₹ symbol
  const cleanCost = (val) => {
    if (!val || val === "-") return "";
  ...
  const cleanDate = (val) => {
    if (!val || val === "-") return "";
    // API returns ISO date strings like "2026-06-26" or "2026-06-26T00:00:00"
  ...
  const [form, setForm] = useState({
    ...
    asset_no:                prefill.asset_no                !== "-" ? prefill.asset_no                : "",
    ...
    // row maps manufacturer_id => "manufacturer" accessor
    manufacturer_id: prefill.manufacturer_id || "",
    serial_no:               prefill.serial_no               !== "-" ? prefill.serial_no               : "",
    system_configuration:    prefill.system_configuration    !== "-" ? prefill.system_configuration    : "",
    // row maps supplier_id => "supplier" accessor
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!prefill.id) {
      toast.error("Asset ID missing — cannot update.");
      return;
    }
    try {
      setLoading(true);
      ...
      if (form.administrator_password.trim() !== "") {
        payload.administrator_password = form.administrator_password;
      }

      if (form.email_password.trim() !== "") {
        payload.email_password = form.email_password;
      }
      await axios.put(API.UPDATE_COMPUTER_ASSET(prefill.id), payload, {
        headers: authHeaders(),
      });

      toast.success("Computer Asset updated successfully");
      navigate("/assets/computer-assets");
    } catch (err) {
      console.error(err);
      toast.error(err?.response?.data?.detail || "Failed to update Computer Asset");
    } finally {
      setLoading(false);
    }
  };
    ...
    <Box sx={{ height: "100%", overflowY: "auto", p: 3, bgcolor: "background.default" }}>
      <Paper elevation={2} sx={{ maxWidth: 900, mx: "auto", borderRadius: 3, overflow: "hidden" }}>
        ...
        <Box sx={{ px: 3, py: 2.5, borderBottom: 1, borderColor: "divider", bgcolor: "background.paper" }}>
          <Typography variant="h6" fontWeight={700}>Update Computer Asset</Typography>
          <Typography variant="caption" color="text.secondary">
            Editing: <strong>{prefill.asset_no || prefill.pc_name || `ID ${prefill.id}`}</strong>
          </Typography>
        </Box>
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ p: 3 }}>
          <Stack spacing={3}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}>
                <TextField size="small" fullWidth label="Asset Number" name="asset_no" value={form.asset_no} onChange={handleChange} />
              </Grid>
              <Grid item xs={12} sm={6}>
            ...
            {/* 2. Client fields — shown only when asset_type === CLIENT */}
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}>
                <TextField size="small" fullWidth label="Serial Number" name="serial_no" value={form.serial_no} onChange={handleChange} />
              </Grid>
              <Grid item xs={12} sm={6}>
            ...
            <Stack direction="row" spacing={1.5} pt={1}>
              ...
              >
                {loading ? "Updating..." : "Update Computer Asset"}
              </Button>
